...
class DrawView extends View {
    ...
    public void draw_myElectric(Canvas canvas, Float power, LinkedHashMap data)
    {
        ...
        canvas.drawText("Current Speed:", 120*scale, (int)120*scale, paint);

        // Power value text
        paint.setColor(Color.parseColor("#d185cd"));
        paint.setTextSize(160*scale);
        canvas.drawText(String.format("%.0f", power)+"Km", 230*scale, 260*scale, paint);

        // km/h text
        paint.setTypeface(robotoNormal);
        paint.setTextSize((int)35*scale);
        canvas.drawText("USE TODAY: 0.5 Km/h", 100*scale, (int)320*scale, paint);
        ...
        // Draw Axes
        paint.setColor(Color.rgb(209, 133, 205));
        ...
        // Draw km/h label top-left
        paint.setTextSize(35*scale);
        canvas.drawText("Km", left+10, top+30*scale, paint);
        ...
        while(keySetIterator.hasNext()){
            ...
            paint.setColor(Color.rgb(153, 68, 148));
            canvas.drawRect(barLeft,barTop,barRight,barBottom,paint);

            // Draw km/h label text
            if (py>38*scale) {
